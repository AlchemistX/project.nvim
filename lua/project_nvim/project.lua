...
function M.find_pattern_root()
  ...
  local function sub(dir, identifier)
    local parent_dir = get_parent(dir)
    while true do
      if is(parent_dir, identifier) then
        return true
      end
      local current = parent_dir
      parent_dir = get_parent(parent_dir)
      if current == parent_dir then
        return false
      end
    end
  end
  ...
  local function child(dir, identifier)
    local parent_dir = get_parent(dir)
    return is(parent_dir, identifier)
  end
